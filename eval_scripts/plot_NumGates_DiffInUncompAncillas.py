...
if __name__ == '__main__':
    ...
    config_path = 'configs/default_config_gates.yaml'
    if len(sys.argv) < 2:
    ...
    for a in range(num_a_min, num_a_max+num_a_step, num_a_step):
        var_gates_result_dict = {}
        for g in range(num_g_min, num_g_max+num_g_step, num_g_step):
            filled_results = get_ancilla_metrics(num_q=num_q, num_g=g, num_a=a, 
                                             results=NumAncillaUncomped(), num_circuits=num_circuits)
            var_gates_result_dict.update({g:filled_results})    

        plot_ancilla_results(var_gates_result_dict, xlabel='Number of Gates',
                             figname=f'Plot_num_ancillas_uncomputed_with_gates_{num_q}q_{a}a_{num_g_min}-{num_g_max}g')
